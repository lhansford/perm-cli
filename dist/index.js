...
const fs$1 = require$$3__default["default"];
...
const PEOPLE_DIR = process.env.PERM_PEOPLE_DIR || './.perm/people';

function splitTitleAndContent(content) {
...
function getMarkdownFilesInDirectory(directory) {
    const files = require$$3.readdirSync(directory, { withFileTypes: true });
    return files.filter((result) => !result.isDirectory() && result.name.endsWith('.md'));
}
function getAndParseMarkdownFile(path) {
    const contents = require$$3.readFileSync(path, { encoding: 'utf8' });
...
function birthdays() {
    getPeople()
        ...
        .forEach(({ person, birthDay }) => console.log(`${person.name} - ${differenceInDays(birthDay, new Date())} days to go (${birthDay.toDateString()})`));
}
...
function getDaysOverdue({ lastContact, contactFrequency }) {
